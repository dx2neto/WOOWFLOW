...
Deno.serve(async (req) => {
  try {
    ...
    // Dedup apenas por lembretes já ENVIADOS (permite retentar falhas e reduz o volume lido).
    const alreadySent = await base44.asServiceRole.entities.ReminderLog.filter({ status: 'enviado' });
    ...
    const base44 = createClientFromRequest(req);
    await logError(base44, 'sendPaymentReminders', error, { action: 'run', severity: 'alta' });
