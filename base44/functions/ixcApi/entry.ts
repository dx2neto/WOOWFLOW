...
Deno.serve(async (req) => {
  try {
    ...
    if (action === 'update_contract') {
      ...
      if (!bodyData) return Response.json({ success: false, error: 'Dados do contrato são obrigatórios' }, { status: 400 });
      const { res, data } = await ixcWrite(`cliente_contrato/${contratoId}`, bodyData, 'PUT');
      if (!res.ok) return Response.json({ success: false, error: 'Falha ao atualizar contrato no IXC', details: data }, { status: res.status });
      await base44.asServiceRole.entities.IntegrationLog.create({ integration: 'ixcApi', action: 'update_contract', status: 'sucesso', details: `contrato ${contratoId}` });
      return Response.json({ success: true, data, message: 'Contrato atualizado com sucesso' });
    }
    ...
    // ── PRÉ-ANÁLISE DO CLIENTE (busca por telefone, agrega tudo) ───────────────
    if (action === 'pre_analise' || action === 'pre_analysis') {
      const rawPhone = String(search || bodyData?.phone || '').replace(/\D/g, '');
      if (!rawPhone || rawPhone.length < 8) return Response.json({ success: false, error: 'Telefone inválido para pré-análise' }, { status: 400 });

      // Tenta buscar por telefone_celular e por fone (com e sem DDI 55)
      const phoneVariants = [rawPhone, rawPhone.replace(/^55/, '')];
      let clienteRaw: any = null;
      for (const variant of phoneVariants) {
        for (const field of ['cliente.telefone_celular', 'cliente.fone']) {
          const { res, data } = await ixcPost('cliente', { qtype: field, query: variant, oper: 'L', page: '1', rp: '5', sortname: 'cliente.id', sortorder: 'desc' });
          if (res.ok && (data.registros || []).length > 0) { clienteRaw = data.registros[0]; break; }
        }
        if (clienteRaw) break;
      }

      if (!clienteRaw) return Response.json({ success: true, data: { found: false, message: 'Cliente não encontrado na base do IXC' } });
      ...
      // Faturas em aberto
      const { res: fatRes, data: fatData } = await ixcPost('fn_areceber', { qtype: 'fn_areceber.id_cliente', query: clientId, oper: '=', sortname: 'fn_areceber.data_vencimento', sortorder: 'asc', page: '1', rp: '100' });
      const hoje = new Date().setHours(0, 0, 0, 0);
      ...
      const pppoe = (pppoeRes.ok ? pppoeData.registros || [] : []).map((r: any) => ({
        login: r.login || '', password: r.senha || '', ip: r.ip || '', status: r.ativo === 'S' ? 'online' : 'offline',
      }));
      ...
      await base44.asServiceRole.entities.IntegrationLog.create({ integration: 'ixcApi', action: 'pre_analise', status: 'sucesso', details: `cliente ${clientId}` });
      return Response.json({
        success: true,
        data: {
          found: true, cliente, contratos, faturas: { abertas: faturasAbertas.length, vencidas: faturasVencidas.length, total_devido: totalDevido, risk: financial_risk },
          pppoe, tickets, summary: {
            is_active: cliente.is_active,
            has_overdue: faturasVencidas.length > 0,
            overdue_count: faturasVencidas.length,
            total_devido: totalDevido,
            financial_risk,
            contracts_count: contratos.length,
            active_contracts: contratos.filter((c) => c.status === 'ativo').length,
    ...
    const base44 = createClientFromRequest(req);
    await logError(base44, 'ixcApi', error, { action: action || 'unknown', severity: 'alta' });
