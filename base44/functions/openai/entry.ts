...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const baseUrl = String(integration.base_url || 'https://api.openai.com/v1').replace(/\/+$/, '');
    const apiKey = Deno.env.get('OPENAI_API_KEY') || '';
    if (!apiKey) return Response.json({ success: false, error: 'OPENAI_API_KEY não configurado' }, { status: 500 });
    ...
    switch (action) {
      ...
      case 'createEmbedding':
        url = `${baseUrl}/embeddings`; method = 'POST';
        reqBody = JSON.stringify(params);
        break;
      case 'createImage':
